...
EXPECTED_CURRENT = "zyicl1u0.up.railway.app"
ROLLBACK_TARGET = "custom-domains.chatgpt.site"
...
if current == ROLLBACK_TARGET:
    print("Cloudflare DNS is already rolled back to ChatGPT Sites.")
elif current == EXPECTED_CURRENT:
    payload = {
        "type":"CNAME","name":HOST,"content":ROLLBACK_TARGET,
        "ttl":1,"proxied":False,
        "comment":"Broast Today restored to ChatGPT Sites while Railway custom domain is diagnosed",
    }
    out=request(BASE + "/" + record["id"], method="PUT", body=payload)
    if not out.get("success"):
        raise SystemExit("Rollback failed")
    print("Cloudflare DNS rollback to ChatGPT Sites completed successfully.")
else:
